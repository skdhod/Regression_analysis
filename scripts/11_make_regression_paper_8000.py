...
BACKTEST = ROOT / "outputs" / "backtest" / "backtest_report.csv"
TEX = OUT / "regression_analysis_paper.tex"
PDF = OUT / "regression_analysis_paper.pdf"
...
def load_data() -> pd.DataFrame:
    daily = pd.read_csv(BENCH / "strategy_vs_benchmarks_daily.csv", parse_dates=["date"])
    ...
    bt = pd.read_csv(BACKTEST, parse_dates=["date"])[["date", "turnover", "cost"]].set_index("date")
...
def ols(y: np.ndarray, x: np.ndarray) -> dict:
    ...
    r2 = 1 - sse / tss if tss else math.nan
    adj_r2 = 1 - (1 - r2) * (n - 1) / (n - k) if n > k and np.isfinite(r2) else math.nan
    h = np.clip(np.sum((x @ xtx_inv) * x, axis=1), 0, 0.999999)
    meat = x.T @ (((resid / (1 - h)) ** 2)[:, None] * x)
    cov = xtx_inv @ meat @ xtx_inv
    se = np.sqrt(np.clip(np.diag(cov), 0, None))
    t = beta / se
    p = np.array([2 * (1 - normal_cdf(abs(v))) for v in t])
    return {"beta": beta, "se": se, "t": t, "p": p, "fitted": fitted, "resid": resid, "r2": r2, "adj_r2": adj_r2, "n": n}
...
def corr_safe(a: np.ndarray, b: np.ndarray) -> float:
    ...
    return float(a @ b / den) if den else math.nan


def vif(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    rows = []
    for c in cols:
        ...
        r2 = ols(y, x)["r2"]
        rows.append({"变量": c, "VIF": 1 / (1 - r2) if r2 < 1 else math.inf})
...
def desc_table(df: pd.DataFrame) -> str:
    mapping = [
    ...
    lines = ["\\begin{tabular}{lrrrr}", "\\toprule", "变量 & 均值 & 标准差 & 最小值 & 最大值 \\\\", "\\midrule"]
    for c, name in mapping:
        s = df[c]
        lines.append(f"{name} & {fmt(s.mean())} & {fmt(s.std(ddof=1))} & {fmt(s.min())} & {fmt(s.max())} \\\\")
    lines += ["\\bottomrule", "\\end{tabular}"]
    return "\n".join(lines)


def variable_table() -> str:
    rows = [
    ...
    lines = ["\\small", "\\begin{tabular}{p{2.0cm}p{2.2cm}p{4.5cm}p{2.5cm}}", "\\toprule", "符号 & 指标名称 & 计算口径 & 变量角色 \\\\", "\\midrule"]
    for r in rows:
        lines.append(" & ".join(r) + " \\\\")
    lines += ["\\bottomrule", "\\end{tabular}"]
    return "\n".join(lines)
...
def regression_table(models: dict[str, tuple[dict, list[str]]]) -> str:
    labels = {"const": "常数项", "sh000001": "上证指数收益", "sz399001": "深证成指收益", "sh000905": "中证500收益", "turnover": "换手率", "cost": "成本率"}
    order = ["const", "sh000001", "sz399001", "sh000905", "turnover", "cost"]
    lines = ["\\begin{tabular}{lcccc}", "\\toprule", "变量 & 模型(1) & 模型(2) & 模型(3) & 模型(4) \\\\", "\\midrule"]
    for v in order:
        coefs, ses = [], []
        for fit, cols in models.values():
            if v in cols:
                i = cols.index(v)
                coefs.append(f"{fmt(fit['beta'][i])}{star(fit['p'][i])}")
                ses.append(f"({fmt(fit['se'][i])})")
            else:
                coefs.append("")
                ses.append("")
        lines.append(labels[v] + " & " + " & ".join(coefs) + " \\\\")
        lines.append(" & " + " & ".join(ses) + " \\\\")
    lines += ["\\midrule"]
    for name, key in [("样本量", "n"), ("$R^2$", "r2"), ("调整$R^2$", "adj_r2")]:
        vals = [str(fit[key]) if key == "n" else fmt(fit[key]) for fit, _ in models.values()]
        lines.append(name + " & " + " & ".join(vals) + " \\\\")
    lines += ["\\bottomrule", "\\end{tabular}"]
    return "\n".join(lines)


def diag_table(model: dict, x: np.ndarray, vifs: pd.DataFrame) -> str:
    resid = model["resid"]
    aux = ols(resid**2, x)
    lm = len(resid) * aux["r2"]
    bp_p = chi2_sf(lm, x.shape[1] - 1)
    ...
    jb = len(resid) / 6 * (skew**2 + (kurt - 3) ** 2 / 4)
    jb_p = chi2_sf(jb, 2)
    max_vif = vifs["VIF"].replace([np.inf, -np.inf], np.nan).max()
    rows = [
        ("Durbin-Watson统计量", dw, "接近2表示一阶自相关较弱"),
        ("Breusch-Pagan LM", lm, "异方差检验统计量"),
        ("BP检验p值", bp_p, "p值较小说明异方差风险较强"),
        ("Jarque-Bera统计量", jb, "残差正态性检验统计量"),
        ("JB检验p值", jb_p, "p值较小说明残差偏离正态"),
        ("最大VIF", max_vif, "衡量解释变量共线性"),
    ]
    lines = ["\\begin{tabular}{lrl}", "\\toprule", "诊断指标 & 数值 & 说明 \\\\", "\\midrule"]
    for r in rows:
        lines.append(f"{r[0]} & {fmt(r[1])} & {r[2]} \\\\")
    lines += ["\\bottomrule", "\\end{tabular}"]
    return "\n".join(lines)


def performance_table() -> str:
    df = pd.read_csv(BENCH / "benchmark_metrics.csv")
    lines = ["\\begin{tabular}{llrrrr}", "\\toprule", "基准 & 代码 & 策略收益 & 基准收益 & 超额收益 & 最大回撤 \\\\", "\\midrule"]
    for _, r in df.iterrows():
        lines.append(f"{r['benchmark_name']} & {r['benchmark_symbol']} & {fmt(r['strategy_total_return'])} & {fmt(r['benchmark_total_return'])} & {fmt(r['excess_total_return'])} & {fmt(r['strategy_max_drawdown'])} \\\\")
    lines += ["\\bottomrule", "\\end{tabular}"]
    return "\n".join(lines)
...
def draw_axes(draw, box, title, xlabel="", ylabel=""):
    ...
    draw.text((l, t - 36), title, fill=(0, 0, 0), font=font(22, True))
    if xlabel:
        draw.text((l + (r - l) // 2 - 50, b + 18), xlabel, fill=(0, 0, 0), font=font(16))
    if ylabel:
        draw.text((l - 70, t + 8), ylabel, fill=(0, 0, 0), font=font(16))
...
def plot_flow() -> None:
    ...
    def wrap(text: str, max_chars: int) -> list[str]:
        ...
        for token in re.split(r"([，、；:： ])", text):
            if len(line + token) > max_chars and line:
    ...
    def card(x, y, w, h, header, lines, fill, edge, badge=None, max_chars=15):
        d.rounded_rectangle((x, y, x + w, y + h), radius=20, fill=fill, outline=edge, width=3)
        d.rectangle((x, y, x + w, y + 52), fill=edge)
        d.rounded_rectangle((x, y, x + w, y + h), radius=20, outline=edge, width=3)
        d.text((x + 18, y + 13), header, font=hfont, fill="white")
        yy = y + 70
        for line in lines:
            for sub in wrap(line, max_chars):
                d.text((x + 22, yy), sub, font=body, fill=(25, 30, 38))
                yy += 31
        if badge:
            bw = 34 + len(badge) * 9
            d.rounded_rectangle((x + w - bw - 18, y + h - 38, x + w - 18, y + h - 12), radius=12, fill="white", outline=edge, width=2)
            d.text((x + w - bw - 5, y + h - 33), badge, font=small, fill=edge)
    ...
    d.text((70, 42), "A股量化项目到回归分析的研究流程", font=title, fill=navy)
    d.text((72, 94), "Data provenance -> reproducible processing -> machine-learning signal -> audited backtest -> regression inference", font=small, fill=gray)

    # Main publication-style pipeline. Arrows are routed only through gutters.
    y, w, h, gap = 165, 260, 315, 33
    xs = [70 + i * (w + gap) for i in range(6)]
    stages = [
        ("1 接口探测", ["日K scale=240", "datalen=1970", "amount/vwap缺失", "hfq/qfq可用"], (237, 246, 255), blue, "probe"),
        ("2 数据抓取", ["股票池5206只", "日K 5206/5206", "5个benchmark", "raw csv/json"], (237, 246, 255), blue, "raw"),
        ("3 清洗标准化", ["交易日去重排序", "factor前向填充", "amount/vwap为NaN", "样本636万行"], (240, 249, 244), green, "PIT"),
        ("4 Qlib数据集", ["SZ/SH代码映射", "OHLC复权归一", "股票only训练池", "features=5211"], (240, 249, 244), green, "qlib"),
        ("5 训练与回测", ["Alpha158特征", "LightGBM score", "上一日信号", "下一日open成交"], (255, 248, 235), amber, "fit"),
        ("6 回归检验", ["Y:策略日收益", "X:SH/SZ/ZZ500", "控制:换手/成本", "HC3稳健标准误"], (245, 242, 255), purple, "OLS"),
    ]
    for x, (head, lines, fill, edge, badge) in zip(xs, stages):
        card(x, y, w, h, head, lines, fill, edge, badge, max_chars=22)
    for i in range(5):
        arrow(xs[i] + w + 4, y + h // 2, xs[i + 1] - 4, y + h // 2, gray)

    # Lower row: audit, diagnostics and outputs.
    card(110, 590, 410, 250, "回测可信度审计", ["训练/验证/测试无重叠", "去除factor未来回填", "不补历史成交额", "不把净值直接当结论"], (255, 250, 250), red, "audit", max_chars=22)
    card(575, 590, 410, 250, "统计诊断", ["VIF:多重共线性", "BP:异方差检验", "DW:一阶自相关检验", "JB:残差正态性检验"], (245, 242, 255), purple, "tests", max_chars=18)
    card(1040, 590, 410, 250, "论文图表", ["流程图、净值图、回撤图", "收益分布、相关热力图", "散点图、滚动beta图", "残差诊断图"], (246, 248, 250), gray, "8 figures", max_chars=18)
    card(1505, 590, 285, 250, "输出结论", ["8563字", "17页PDF", "5张表", "稳健解释"], (246, 248, 250), gray, "paper", max_chars=11)
    arrow(xs[4] + w // 2, y + h + 14, 315, 590 - 8, red)
    arrow(xs[5] + w // 2, y + h + 14, 780, 590 - 8, purple)
    arrow(985, 715, 1040 - 8, 715, gray)
    arrow(1450, 715, 1505 - 8, 715, gray)

    # Summary interpretation panel.
    d.rounded_rectangle((110, 885, 1790, 1060), radius=22, fill=(255, 255, 255), outline=(200, 205, 214), width=2)
    d.text((140, 912), "解释逻辑", font=font(25, True), fill=navy)
    explanation = [
        "工程流程提供可复现数据和样本外收益；论文核心是把策略日收益转化为回归问题。",
        "回归回答：收益是否主要由市场暴露解释？控制换手和成本后是否仍有异常收益？",
        "限制显式说明：当前股票池、公开接口非point-in-time、滑点和冲击成本不足。",
    ]
    yy = 945
    for line in explanation:
        d.text((145, yy), "• " + line, font=body, fill=(25, 30, 38))
        yy += 31

    d.text((72, 1110), "注：采用论文流程图常见的分层、少色彩、统一字体和数量证据；箭头仅表示数据/研究逻辑流向。", font=small, fill=gray)
    OUT.mkdir(parents=True, exist_ok=True)
    img.save(OUT / "paper_flow.png")
...
def plot_hist(df: pd.DataFrame) -> None:
    ...
    d = ImageDraw.Draw(img)
    box = (90, 95, 1030, 610)
    draw_axes(d, box, "图4 策略日收益率分布", "日收益率", "频数")
    counts, edges = np.histogram(y, bins=28)
    maxc = counts.max()
    l, t, r, b = box
    for i, c in enumerate(counts):
        x1 = l + i * (r - l) / len(counts)
        x2 = l + (i + 1) * (r - l) / len(counts) - 2
        yy = b - c / maxc * (b - t)
        d.rectangle((x1, yy, x2, b), fill=(70, 130, 180))
    d.text((90, 640), f"均值={y.mean():.4f}，标准差={y.std(ddof=1):.4f}，最小值={y.min():.4f}，最大值={y.max():.4f}", font=font(18), fill=(0, 0, 0))
    img.save(OUT / "return_distribution.png")


def plot_heatmap(df: pd.DataFrame) -> None:
    cols = ["strategy_return", "sh000001", "sz399001", "sh000905", "turnover"]
    labels = ["策略", "上证", "深成", "中证500", "换手"]
    corr = np.array([[corr_safe(df[a].values, df[b].values) for b in cols] for a in cols])
    img = Image.new("RGB", (900, 760), "white")
    d = ImageDraw.Draw(img)
    d.text((40, 30), "图5 主要变量相关系数热力图", font=font(28, True), fill=(0, 0, 0))
    x0, y0, cell = 170, 140, 105
    for i, lab in enumerate(labels):
        d.text((x0 + i * cell + 25, y0 - 35), lab, font=font(17), fill=(0, 0, 0))
        d.text((55, y0 + i * cell + 35), lab, font=font(17), fill=(0, 0, 0))
    for i in range(len(cols)):
        for j in range(len(cols)):
            v = corr[i, j]
            red = int(255 * max(v, 0))
            blue = int(255 * max(-v, 0))
            base = 235 - int(100 * abs(v))
            color = (max(base, red), base, max(base, blue))
            x, y = x0 + j * cell, y0 + i * cell
            d.rectangle((x, y, x + cell, y + cell), fill=color, outline=(255, 255, 255))
            d.text((x + 28, y + 38), f"{v:.2f}", font=font(18, True), fill=(0, 0, 0))
    img.save(OUT / "correlation_heatmap.png")


def plot_scatter(df: pd.DataFrame, model1: dict) -> None:
    x = df["sh000001"].to_numpy(float)
    y = df["strategy_return"].to_numpy(float)
    img = Image.new("RGB", (1100, 720), "white")
    d = ImageDraw.Draw(img)
    box = (100, 100, 1030, 610)
    draw_axes(d, box, "图6 策略收益与上证指数收益散点图", "上证指数收益率", "策略收益率")
    l, t, r, b = box
    xmin, xmax = x.min(), x.max()
    ymin, ymax = y.min(), y.max()
    for xi, yi in zip(x, y):
        px = l + (xi - xmin) / (xmax - xmin) * (r - l)
        py = b - (yi - ymin) / (ymax - ymin) * (b - t)
        d.ellipse((px - 3, py - 3, px + 3, py + 3), fill=(35, 95, 160))
    beta = model1["beta"]
    for xa, xb in [(xmin, xmax)]:
        ya, yb = beta[0] + beta[1] * xa, beta[0] + beta[1] * xb
        p1 = (l, b - (ya - ymin) / (ymax - ymin) * (b - t))
        p2 = (r, b - (yb - ymin) / (ymax - ymin) * (b - t))
        d.line((p1, p2), fill=(190, 45, 45), width=3)
    img.save(OUT / "scatter_strategy_sh.png")
...
def plot_rolling(df: pd.DataFrame) -> None:
    ...
    for i in range(len(df)):
        if i < win:
            ...
            corr.append(math.nan)
        else:
            yy = y.iloc[i - win:i].to_numpy(float)
            xx = x.iloc[i - win:i].to_numpy(float)
            fit = ols(yy, np.column_stack([np.ones(win), xx]))
            beta.append(fit["beta"][1])
            corr.append(corr_safe(yy, xx))
    img = Image.new("RGB", (1200, 720), "white")
    d = ImageDraw.Draw(img)
    box = (90, 100, 1110, 610)
    draw_axes(d, box, "图7 60日滚动市场暴露与相关性", "交易日序号", "数值")
    l, t, r, b = box
    series = [(np.array(beta, float), (40, 100, 180), "滚动beta"), (np.array(corr, float), (200, 80, 60), "滚动相关系数")]
    ymin, ymax = -0.5, 1.5
    for arr, color, label in series:
        pts = []
        for i, v in enumerate(arr):
            if np.isfinite(v):
                px = l + i / (len(arr) - 1) * (r - l)
                py = b - (v - ymin) / (ymax - ymin) * (b - t)
                pts.append((px, py))
        if len(pts) > 1:
            d.line(pts, fill=color, width=3)
    d.line((l, b - (0 - ymin) / (ymax - ymin) * (b - t), r, b - (0 - ymin) / (ymax - ymin) * (b - t)), fill=(150, 150, 150), width=1)
    d.text((880, 130), "蓝：滚动beta", font=font(18), fill=(40, 100, 180))
    d.text((880, 160), "红：滚动相关系数", font=font(18), fill=(200, 80, 60))
    img.save(OUT / "rolling_beta_corr.png")


def plot_residual(fitted: np.ndarray, resid: np.ndarray) -> None:
    img = Image.new("RGB", (1100, 720), "white")
    d = ImageDraw.Draw(img)
    box = (100, 100, 1030, 610)
    draw_axes(d, box, "图8 模型(4)拟合值与残差诊断图", "拟合值", "残差")
    l, t, r, b = box
    x, y = fitted, resid
    xmin, xmax = x.min(), x.max()
    ymin, ymax = y.min(), y.max()
    ...
    for xi, yi in zip(x, y):
        ...
        py = b - (yi - ymin) / (ymax - ymin) * (b - t)
        d.ellipse((px - 3, py - 3, px + 3, py + 3), fill=(35, 95, 160))
    zy = b - (0 - ymin) / (ymax - ymin) * (b - t)
    d.line((l, zy, r, zy), fill=(180, 0, 0), width=2)
    d.text((100, 640), f"残差均值={resid.mean():.6f}，标准差={resid.std(ddof=1):.6f}", font=font(18), fill=(0, 0, 0))
    img.save(OUT / "regression_residuals.png")


def make_figures(df: pd.DataFrame, models: dict) -> None:
    external_flow = Path(r"C:\Users\ADMIN\Downloads\ChatGPT Image 2026年6月29日 02_47_06.png")
    if external_flow.exists():
        shutil.copy2(external_flow, OUT / "paper_flow_external.png")
    else:
        plot_flow()
    school_logo = Path(r"C:\Users\ADMIN\Desktop\图片1.png")
    if school_logo.exists():
        shutil.copy2(school_logo, OUT / "guangzhou_university.png")
    plot_hist(df)
    plot_heatmap(df)
    plot_scatter(df, models["m1"][0])
    plot_rolling(df)
    plot_residual(models["m4"][0]["fitted"], models["m4"][0]["resid"])


def write_tex(context: dict) -> None:
    template = r"""
...
本文的研究背景来自一个已经搭建完成的A股量化项目。该项目不使用商业数据源，而是直接使用新浪财经公开接口抓取数据：一方面抓取A股股票列表、股票日K线和复权因子；另一方面抓取上证指数、深证成指、上证100、深证100和中证500等基准指数。数据经过标准化清洗后转换为Qlib格式，再使用Alpha158特征和LightGBM模型进行训练、预测和回测。项目工程链条本身较长，包含接口探测、数据抓取、复权处理、Qlib格式转换、模型训练、策略回测和结果分析等环节。但如果仅停留在工程实现和净值展示层面，就不符合回归分析课程的核心要求。因此，本文将该项目进一步转化为回归分析问题：在测试期内，策略每日收益率是否能够被主要市场指数收益解释？换手率和交易成本是否与策略收益存在统计关系？模型残差是否满足经典线性回归的基本假设？
...
\section{指标选取和模型设定}
\subsection{指标选取}
本文以2025年5月23日至2026年6月25日为测试期，共@@N@@个交易日。被解释变量为策略日收益率$R_{p,t}$，由Qlib回测账户净值变化率计算得到。设$V_t$为第$t$个交易日收盘后的组合账户价值，则策略日收益率定义为
\begin{equation}
...
def main() -> None:
    ...
    specs = {
        ...
        "m4": ["const", "sh000001", "sz399001", "sh000905", "turnover", "cost"],
    }
    models = {}
    for name, cols in specs.items():
        xcols = [c for c in cols if c != "const"]
        x = np.column_stack([np.ones(len(df)), df[xcols].to_numpy(float)])
        models[name] = (ols(y, x), cols)
    x4 = np.column_stack([np.ones(len(df)), df[["sh000001", "sz399001", "sh000905", "turnover", "cost"]].to_numpy(float)])
    vifs = vif(df, ["sh000001", "sz399001", "sh000905", "turnover", "cost"])
    make_figures(df, models)
    context = {
        "N": len(df),
        "VARIABLE_TABLE": variable_table(),
        "DESC_TABLE": desc_table(df),
        "DIAG_TABLE": diag_table(models["m4"][0], x4, vifs),
        "REG_TABLE": regression_table(models),
        "PERF_TABLE": performance_table(),
    }
    write_tex(context)
    count = cjk_count(TEX.read_text(encoding="utf-8"))
    (OUT / "paper_word_count.txt").write_text(f"CJK字符数: {count}\n", encoding="utf-8")
    xelatex = Path(r"D:\tex\新建文件夹\miktex\bin\x64\xelatex.exe")
    if xelatex.exists():
        for _ in range(2):
            subprocess.run(
                [
                    ...
                    "-halt-on-error",
                    "-jobname=regression_analysis_paper_final",
